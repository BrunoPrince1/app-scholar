...
export default function ListaAlunosScreen() {
  ...
  // Dropdowns IBGE dentro do modal
  const [estados, setEstados] = useState([]);
  const [cidades, setCidades] = useState([]);
  const [mostrarEstados, setMostrarEstados] = useState(false);
  const [mostrarCidades, setMostrarCidades] = useState(false);

  // Recarrega sempre que a tela ganhar foco (volta do cadastro)
  useFocusEffect(
    useCallback(() => {
      carregarAlunos();
    }, [])
  ...
  const handleSelecionarEstado = async (sigla) => {
    ...
    setMostrarEstados(false);
    try {
      const cids = await listarCidadesPorEstado(sigla);
      setCidades(cids);
    } catch {
      setCidades([]);
    }
        ...
        >
          ...
          >
            ...
            <View style={styles.inputWrap}>
              ...
              <TouchableOpacity
                style={styles.picker}
                onPress={() => { setMostrarEstados((v) => !v); setMostrarCidades(false); }}
              >
                  ...
                  <ScrollView style={{ maxHeight: 180 }} nestedScrollEnabled>
                    {estados.map((e) => (
                      <TouchableOpacity
                        ...
                        onPress={() => handleSelecionarEstado(e.sigla)}
                      >
