...
import {
  listarDisciplinas, atualizarDisciplina, deletarDisciplina, listarProfessores,
...
export default function ListaDisciplinasScreen({ navigation }) {
  ...
  const handleBusca = (texto) => {
    ...
    setFiltrados(disciplinas.filter(d =>
  ...
  const excluirDisciplina = async (id) => {
    try {
      ...
      setFiltrados(prev => prev.filter(d => d.id !== id));
    } catch (err) { Alert.alert('Erro', err.message); }
  };

  const semestreColor = (sem) => {
    const n = parseInt((sem || '').replace(/\D/g, '')) || 0;
    return ['#1565C0', '#2E7D32', '#E65100', '#6A1B9A', '#00695C', '#558B2F'][(n - 1) % 6] || '#607D8B';
  };
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      ...
      <View style={styles.cardActions}>
        <TouchableOpacity
          style={[styles.actionBtn, styles.alunosBtn]}
  ...
  return (
    <View style={styles.screen}>
      <View style={styles.searchBar}>
        <TextInput style={styles.searchInput} placeholder="🔍  Buscar por nome, curso, semestre..." value={busca} onChangeText={handleBusca} placeholderTextColor="#999" />
      </View>
      ...
      {loading ? (
        ...
        <View style={styles.empty}>
          ...
          <Text style={styles.emptyText}>{busca ? 'Nenhuma disciplina encontrada.' : 'Nenhuma disciplina cadastrada ainda.'}</Text>
        </View>
      ) : (
        <>
          <Text style={styles.contador}>{filtrados.length} disciplina{filtrados.length !== 1 ? 's' : ''}</Text>
          <FlatList
            data={filtrados}
          ...
          <ScrollView contentContainerStyle={styles.modalBody} keyboardShouldPersistTaps="handled">
            ...
            <View style={styles.inputWrap}>
              ...
              <TouchableOpacity style={styles.picker} onPress={() => setMostrarProfs(v => !v)}>
                <Text style={{ color: profNome ? colors.text : '#aaa' }}>{profNome || 'Selecione um professor'}</Text>
              </TouchableOpacity>
              {mostrarProfs && (
                <View style={styles.dropdown}>
                  <ScrollView style={{ maxHeight: 200 }} nestedScrollEnabled>
                    <TouchableOpacity style={styles.dropItem} onPress={() => { handleChange('professor_id', ''); setProfNome(''); setMostrarProfs(false); }}>
...
const styles = StyleSheet.create({
  ...
  chBox: { backgroundColor: '#E8F5E9', borderRadius: 10, paddingHorizontal: 12, paddingVertical: 8, alignItems: 'center', marginLeft: 8 },
  ...
  cardActions: { flexDirection: 'row', gap: 6, borderTopWidth: 1, borderTopColor: '#f0f0f0', paddingTop: 10, marginTop: 8 },
  actionBtn: { flex: 1, paddingVertical: 9, borderRadius: 8, alignItems: 'center' },
  alunosBtn: { backgroundColor: '#E8F5E9' },
