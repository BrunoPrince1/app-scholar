...
// ── GET /api/boletim/:matricula  (admin) ──────────────────────────────────────
  ...
  try {
    ...
    return res.json({
      ...
      disciplinas: notasR.rows.map(n => ({
        id: n.id, disciplina_id: n.disciplina_id, disciplina: n.disciplina,
        nota1: parseFloat(n.nota1), nota2: parseFloat(n.nota2),
        media: parseFloat(n.media), situacao: n.situacao,
      })),
    });
  } catch (err) {
    console.error('consultarBoletim:', err);
    return res.status(500).json({ erro: 'Erro interno.' });
  }
}

// ── GET /api/meu-boletim  (aluno logado) ─────────────────────────────────────
async function meuBoletim(req, res) {
  try {
    const alunoR = await pool.query('SELECT * FROM alunos WHERE email=$1', [req.usuario.email]);
    if (!alunoR.rows.length) return res.status(404).json({ erro: 'Perfil de aluno não encontrado para este e-mail.' });
    const aluno = alunoR.rows[0];
...
async function disciplinasDoProfessor(req, res) {
  try {
    ...
    if (!profR.rows.length) return res.status(404).json({ erro: 'Perfil de professor não encontrado para este e-mail.' });
    ...
    console.error('disciplinasDoProfessor:', err);
    return res.status(500).json({ erro: 'Erro interno.' });
  }
}

// ── GET /api/professor/disciplinas/:id/alunos  ───────────────────────────────
async function alunosDaDisciplina(req, res) {
  const disciplinaId = parseInt(req.params.id);
  try {
    // Se for admin, acessa direto. Se for professor, valida posse da disciplina.
    if (req.usuario.perfil === 'professor') {
      const profR = await pool.query('SELECT id FROM professores WHERE email=$1', [req.usuario.email]);
      if (!profR.rows.length) return res.status(403).json({ erro: 'Acesso negado.' });

      const discR = await pool.query('SELECT professor_id FROM disciplinas WHERE id=$1', [disciplinaId]);
      if (!discR.rows.length) return res.status(404).json({ erro: 'Disciplina não encontrada.' });

      // Comparação numérica explícita
      if (parseInt(discR.rows[0].professor_id) !== parseInt(profR.rows[0].id)) {
        return res.status(403).json({ erro: 'Você não é responsável por esta disciplina.' });
      }
    }

    // Retorna alunos matriculados (com nota lançada) nesta disciplina
    const r = await pool.query(
    ...
    console.error('alunosDaDisciplina:', err);
    return res.status(500).json({ erro: 'Erro interno.' });
  }
}

// ── GET /api/disciplinas/:id/todos-alunos  ───────────────────────────────────
// Retorna TODOS os alunos do sistema com flag indicando se já está na disciplina
async function todosAlunosDaDisciplina(req, res) {
  ...
  try {
    ...
    console.error('todosAlunosDaDisciplina:', err);
    return res.status(500).json({ erro: 'Erro interno.' });
  }
}
...
async function lancarNota(req, res) {
  ...
  if (req.usuario.perfil === 'professor') {
    ...
    if (profR.rows.length) {
      ...
      if (discR.rows.length && parseInt(discR.rows[0].professor_id) !== parseInt(profR.rows[0].id)) {
        return res.status(403).json({ erro: 'Você não é responsável por esta disciplina.' });
      }
    }
  }
